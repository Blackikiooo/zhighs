...
/// Matches the FMA-contracted code emitted for HiGHS productQuad by the
/// benchmark's GCC `-O3 -march=native -flto` configuration. Exact-product
/// callers must use one of the `multiplyHighPrecision*` APIs instead.
pub fn multiplyCompensatedAssumeValid(matrix: csc.CscMatrix, x: []const f64, y: []f64, scratch: []foundation.HCD) void {
    ...
    while (col < ncol) : (col += 1) {
        ...
        while (position < end) : (position += 1) {
            ...
            const old_low = sp[row].low;
            const value = vs[position];
            const new_high = @mulAdd(f64, value, multiplier, old_high);
            const prod_gap = @mulAdd(f64, -value, multiplier, new_high);
            const rounded_prod = new_high - prod_gap;
            const prod_error = @mulAdd(f64, value, multiplier, -rounded_prod);
            const err = (old_high - prod_gap) + prod_error;
            // Store both halves together so LLVM can use one 128-bit store,
            // matching the HighsCDouble code generated by GCC.
            sp[row] = .{ .high = new_high, .low = old_low + err };
        }
    }
    var row: usize = 0;
