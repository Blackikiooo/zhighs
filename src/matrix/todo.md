...
当前执行批次（2026-07-14）：`ModelEditPlan` 第一阶段已完成并接入 `applyPending`。
...
去重；下一批次为 added rows/columns 与 general structural merge。
...
- [ ] added rows/columns 与 name edits 仍需进入有限操作流；当前保留原 pending payload。
...
- [ ] 将 added rows/columns 和 name edits 迁入 plan；这些 payload 当前仍由原
  `PendingChange` ownership 执行。
...
- [ ] 合并 name 更新，并让 `addVar` 后紧随的字段修改直接进入新变量最终初始化数据。
